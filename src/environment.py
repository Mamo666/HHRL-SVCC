...
class Environment:
    ...
    def get_head_cav_obs(self, cav_id):
        ...
        def get_lane_mean_speed(vehicle_id):
            if vehicle_id is not None:
                return self.lane_get_speed(traci.vehicle.getLaneID(cav_id)) / self.max_speed
            else:
                return -1
        ...
        mean_speed = get_lane_mean_speed(cav_id)

        return ego_state + [dx, dv] + get_light_state_to_car(cav_id, use_left_time=True) + [mean_speed]
